...
class CRIMEBOSSMETA_API UMETA_WarManagerComponent : public UActorComponent {
    GENERATED_BODY()
public:
...
public:
    ...
    UFUNCTION(BlueprintCallable)
    void SetBaseAIBossStrength(EMETA_Gang inGang, int32 inNewValue);
    ...
    bool IsTradeVendorHasCooldown(EMETA_TradeVendor inTradeVendor);
    
    UFUNCTION(BlueprintCallable, BlueprintPure)
    bool IsThereGangInAttitudeList(EMETA_GangAttitudeList inAttitudeList, EMETA_Gang inOwnerGang, EMETA_Gang inTargetGang);
    
    UFUNCTION(BlueprintCallable, BlueprintPure)
    bool IsArmyTierChangingConditionAlreadyCompleted(EMETA_Gang inGang, const FMETA_ConditionForArmyTierChanging& inCompletedCondition);
    
    UFUNCTION(BlueprintCallable)
    void InitTradeRelationship();
    
    UFUNCTION(BlueprintCallable, BlueprintPure)
    FMETA_TurfHistory GetTurfHistoryOfDay(int32 inDay);
    ...
    UFUNCTION(BlueprintCallable, BlueprintPure)
    TMap<EMETA_TradeVendor, EMETA_TradeRelationship> GetTradeRelationshipInfo();
    
    UFUNCTION(BlueprintCallable, BlueprintPure)
    EMETA_TradeRelationship GetTradeRelationshipForGang(EMETA_TradeVendor inTradeVendor);
    ...
    UFUNCTION(BlueprintCallable, BlueprintPure)
    EMETA_GangStrategy GetGangStrategy(EMETA_Gang inGang);
    ...
    EMETA_ArmyTier GetGangArmyTier(EMETA_Gang inGang);
    
    UFUNCTION(BlueprintCallable, BlueprintPure)
    float GetChanceToAttackAnyAdjacentTerritory(EMETA_Gang inGang);
    
    UFUNCTION(BlueprintCallable, BlueprintPure)
    int32 GetBaseAIBossStrength(EMETA_Gang inGang);
    
    UFUNCTION(BlueprintCallable)
    TArray<FMETA_EffectOnAIBossStrength> GetAIBossStrengthEffects(EMETA_Gang inGang);
    
    UFUNCTION(BlueprintCallable, BlueprintPure)
    int32 GetAIBossStrengthChangeFromTerritory(EMETA_Gang inGang);
    ...
    UFUNCTION(BlueprintCallable)
    void ChangeBaseAIBossStrength(EMETA_Gang inGang, int32 inChangeValue);
