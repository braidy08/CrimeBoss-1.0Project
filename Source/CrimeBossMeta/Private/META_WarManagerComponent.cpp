...
void UMETA_WarManagerComponent::SetBaseAIBossStrength(EMETA_Gang inGang, int32 inNewValue) {
}
...
bool UMETA_WarManagerComponent::IsTradeVendorHasCooldown(EMETA_TradeVendor inTradeVendor) {
    return false;
}
...
bool UMETA_WarManagerComponent::IsArmyTierChangingConditionAlreadyCompleted(EMETA_Gang inGang, const FMETA_ConditionForArmyTierChanging& inCompletedCondition) {
    return false;
}
...
FMETA_TurfHistory UMETA_WarManagerComponent::GetTurfHistoryOfDay(int32 inDay) {
...
TMap<EMETA_TradeVendor, EMETA_TradeRelationship> UMETA_WarManagerComponent::GetTradeRelationshipInfo() {
    return TMap<EMETA_TradeVendor, EMETA_TradeRelationship>();
}

EMETA_TradeRelationship UMETA_WarManagerComponent::GetTradeRelationshipForGang(EMETA_TradeVendor inTradeVendor) {
...
EMETA_GangStrategy UMETA_WarManagerComponent::GetGangStrategy(EMETA_Gang inGang) {
...
EMETA_ArmyTier UMETA_WarManagerComponent::GetGangArmyTier_Implementation(EMETA_Gang inGang) {
    return EMETA_ArmyTier::Low;
}

float UMETA_WarManagerComponent::GetChanceToAttackAnyAdjacentTerritory(EMETA_Gang inGang) {
    return 0.0f;
}

int32 UMETA_WarManagerComponent::GetBaseAIBossStrength(EMETA_Gang inGang) {
    return 0;
}
...
int32 UMETA_WarManagerComponent::GetAIBossStrengthChangeFromTerritory(EMETA_Gang inGang) {
    return 0;
}
...
void UMETA_WarManagerComponent::ChangeBaseAIBossStrength(EMETA_Gang inGang, int32 inChangeValue) {
}
